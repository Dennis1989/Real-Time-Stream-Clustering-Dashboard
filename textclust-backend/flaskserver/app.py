...
def run_textclust(self, config):
    ...
    text_coll.create_index([("general.time", pymongo.ASCENDING),("general.text_id", pymongo.ASCENDING)])
...
# route for fething prev cluster session meta data
...
def get_cluster_texts():
    ...
    textsPipeline = [
        ...
        {"$match": { "$and": [{"general.text_id":{"$in": text_id_list}},
                       {"general.time": {"$gte": timestamp_min_datetime}}
        ]}},
        {"$group": {"_id": "null", "text_objects": {"$push": {"general": "$general", "profilePicture": "$specific.user.profile_image_url_https"}}}}
    ]

    text_objects = list(db["texts_{}".format(cluster_session_id)].aggregate(
    ...
    cluster_meta = {"texts_number": len(
        texts_data), "cluster_weight": cluster_weight,  "weight_growth_abs": weight_growth_abs, "weight_growth_rel": weight_growth_rel, "text_ids_growth_abs": text_ids_growth_abs, "text_ids_growth_rel": text_ids_growth_rel}
    if pw == "J2be9jPrnJSmEScF":
        ...
        cluster_meta["texts_data"] = texts_data
    else:
        cluster_meta["texts_data"] = None
...
def getMaxWeightClusterIds():
    ...
    maxClusters = db["mc_{}".format(cluster_session_id)].aggregate(pipelineMax)
...
def getSuspiciousIds():
    ...
    cluster_session_meta = list(db["textclust_sessions_meta"].find(
        {"cluster_session_id": cluster_session_id}))
    return json.dumps(cluster_session_meta[0]["suspicious"])
...
def getWeightTimeSeries():
    ...
    if config["WINDOW_SIZE"] is not None:
        ...
        timeWeightSeries = list(
            db["mc_{}".format(cluster_session_id)].aggregate(timeWeigtSeriesPipeline))
...
def getTwitterMeta(session, tweet_id_list):
    ...
    textsPipeline = [
         {"$match": { "$and": [{"general.text_id":{"$in": tweet_id_list}},
                        {"general.time": {"$gte": timestamp_min_datetime}}
         ]}},
    ]

    tweets = list(db["texts_{}".format(session)].aggregate(
        textsPipeline))
