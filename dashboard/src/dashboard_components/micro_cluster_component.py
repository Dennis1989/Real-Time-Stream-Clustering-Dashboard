...
def micro_cluster_layout():
    ...
    figureInit = go.Figure(dict(data=data_init)) \
        .update_layout(
            ...
            xaxis_title="Zeit",
            yaxis_title="Cluster Gewicht",
            legend_title="Top Clusters / Themen",

    )

    figureInit.update_xaxes(fixedrange=True)
                ...
                for i, traceDict in enumerate(current_shown_cluster_traces):
                    ...
                    if cluster_id in to_append_clusters:
                        ...
                        if rtf == "true":
                            ...
                            x_list = x_list + np.timedelta64(2, 'h')
                        else:
                            x_list = x_list + \
                        ...
                        opacity = (current_weight_cluster / max_weight)
